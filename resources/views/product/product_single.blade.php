                        ...
                        <div class="category">
                            <ul>
                                ...
                                @if ($product->subcategory_id != '0')
                                    <li><span class="next"><i class="fas fa-chevron-right"></i></span></li>
                                    <li><a href="{{ url('/store') }}"><i class="far fa-folder"></i>
                                            {{ $product->getSubcategory->name }}</a></li>
                                @endif
                            </ul>
                        </div>
                        <div class="add_cart">
                            {!! Form::open(['url' => '/cart/product/'.$product->id.'/add']) !!}                            
                            {{--  {!! Form::hidden('product_id', null, ['id' => 'field_product_id']) !!}  --}}
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="variants">
                                        <p><strong>Opciones de producto</strong></p>
                                        <ul id="inventory">
                                            @foreach ($product->getInventory as $inventory)
                                                <li><a href="#" class="inventory" id="inventory_{{ $inventory->id }}" data-inventory-id="{{ $inventory->id }}">{{ $inventory->name }} - 
                                                        <span class="price">{{ Config::get('mycms.currency') . number_format($inventory->price, 2, '.', ',') }}</span>
                                                    </a>
                                                </li>
                                            @endforeach
                                        </ul>
                                    ...
                                    <div class="variants hidden btop1 ptop16 mtop16" id="variants_div" >
                                        <p><strong>Más opciones de producto</strong></p>
                                        <ul id="variants"></ul>                                     
                                    </div>                                   
                                </div>
                            </div>
                            ...
                            <div class="before_quantity">
                                ...
                                <div class="row ">
                                    ...
                                    <div class="col-md-4 col-12">
                                        <button type="submit" class="btn btn-success"><i class="fas fa-cart-plus"></i>
                                            Agregar al carrito</button>
                                    </div>
                                    <div class="col-md-4 col-12">
                                        <a href="#" id="favorite_1_{{ $product->id }}"  onclick="add_to_favorites({{ $product->id }}, '1'); return false;" class="btn btn-favorite"> <i class="fas fa-heart" ></i>                                         
                                            Agregar a Favoritos
                                        </a>
                                    </div>
                                </div>
