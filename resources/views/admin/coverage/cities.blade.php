    ...
    <div class="container-fluid">
        <div class="row">
                ...
                @if (kvfj(Auth::user()->permissions, 'coverage_add'))
                    <div class="panel shadow">
                        ...
                        <div class="inside">
                            ...
                            <label for="name" class="mtop16">Valor de envio</label>
                            <div class="input-group">
                                <span class="input-group-text" id="basic-addon1">
                                    <i class="far fa-keyboard"></i>
                                </span>
                                {!! Form::number('shipping_value', Config::get('mycms.shipping_default_value'), ['class' => 'form-control', 'min' => '0', 'step' => 'any']) !!}
                            </div>

                            <label for="name" class="mtop16">Dìas estimados de entrega</label>
                            <div class="input-group">
                                <span class="input-group-text" id="basic-addon1">
                                    <i class="far fa-keyboard"></i>
                                </span>
            ...
            <div class="col-md-9">
                <div class="panel shadow">
                    <div class="header">
                        <h2 class="title"><i class="fas fa-shipping-fast"></i> Ciudades de: {{ $state->name }}
                        </h2>
                    </div>
                    <div class="inside">
                        <table class="table mtop16">
                            <thead>
                                <tr>
                                    ...
                                    <td><strong>Valor de envio</strong></td>
                            ...
                            <tbody>
                                @foreach ($cities as $city)
                                    <tr>
                                        ...
                                        <td>{{ $city->name }}</td>
                                        <td>{{ config('mycms.currency') }}{{ $city->price }}</td>
                                        <td>{{ $city->days }} días</td>
                                        <td>
                                            <div class="opts">
                                                @if (kvfj(Auth::user()->permissions, 'coverage_edit'))
                                                    <a href="{{ url('/admin/coverage/city/' . $city->id . '/edit') }}"
                                                        data-toggle="tooltip" data-placement="top" title="Editar" class="edit">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                @endif
                                                @if (kvfj(Auth::user()->permissions, 'coverage_delete'))
                                                    <a href="{{ url('/admin/coverage/' . $city->id . '/delete') }}"
                                                        data-action="delete" data-path="admin/coverage"
                                                        data-object="{{ $city->id }}" data-toggle="tooltip"
